...
export default class MessageHandler {
  ...
  handleMessage = async (M: ISimplifiedMessage): Promise<void> => {
    ...
    if (!command)
      return void M.reply(
        `🐰𝘉𝘢𝘬𝘬𝘢, 𝘵𝘩𝘢𝘵'𝘴 𝘸𝘳𝘰𝘯𝘨 𝘤𝘰𝘮𝘮𝘢𝘯𝘥. 𝘶𝘴𝘦 *${this.client.config.prefix}𝘩𝘦𝘭𝘱*.`
      );
    ...
    if (state)
      return void M.reply(
        `✖ 𝘉𝘢𝘬𝘬𝘢, 𝘵𝘩𝘪𝘴 𝘤𝘰𝘮𝘮𝘢𝘯𝘥 𝘪𝘴 𝘥𝘪𝘴𝘢𝘣𝘭𝘦𝘥${
    ...
    if (
      ...
      return void M.reply(`𝘰𝘯𝘭𝘺 𝘮𝘺 𝘴𝘦𝘯𝘱𝘢𝘪 𝘤𝘢𝘯 𝘶𝘴𝘦 𝘵𝘩𝘪𝘴 𝘤𝘰𝘮𝘮𝘢𝘯𝘥..`);
    }
    if (command.config?.adminOnly && !M.sender.isAdmin)
      return void M.reply(
        `𝘬𝘯𝘰𝘸 𝘺𝘰𝘶𝘳 𝘱𝘭𝘢𝘤𝘦. 𝘵𝘩𝘪𝘴 𝘤𝘰𝘮𝘮𝘢𝘯𝘥 𝘪𝘴 𝘮𝘦𝘢𝘯𝘵 𝘧𝘰𝘳 𝘨𝘳𝘰𝘶𝘱 𝘢𝘥𝘮𝘪𝘯𝘴!`
      );
    try {
  ...
  handleState = async (): Promise<void> => {
    const text = `𝘳𝘦𝘤𝘰𝘯𝘯𝘦𝘤𝘵𝘦𝘥🐰 (𝑀𝒶𝒾 𝓈𝒶𝓃)Prefix /`;
    await this.client.sendMessage(
